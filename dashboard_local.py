...
SNAPSHOT_SQL = """
select *
from inventory_sandbox.supplychain.frontlinehealthdashboard
...
def load_snapshot(target_date: str):
    ...
    if "datekey" in df.columns:
        df = df.rename(columns={"datekey": "calendardate"})
    return df
    ...
    if len(trend_range) == 2:
        ...
        if not df_trend.empty:
            ...
            if "websiteunit" in df_trend.columns:
                df_trend["_website_flag"] = (df_trend["websiteunit"] >= 1).astype(int) * df_trend["frontline"].clip(upper=1)
            if "frontline" in df_trend.columns:
                df_trend["_dup_excess"] = (df_trend["frontline"] - 1).clip(lower=0)

            # Compute merch_deviation at store-day level
            merch_dev = None
            if "finalmerchmix" in df_trend.columns and "frontline" in df_trend.columns:
                fmm = pd.to_numeric(df_trend["finalmerchmix"], errors="coerce").fillna(0)
                store_day_total = df_trend.groupby(["calendardate", "parentcostcenterdesc"])["frontline"].transform("sum")
                actual_pct = df_trend["frontline"] / store_day_total.replace(0, pd.NA) * 100
                target_pct = fmm * 100
                df_trend["_merch_abs_dev"] = (actual_pct - target_pct).abs()
                merch_dev = df_trend.groupby(["calendardate", "parentcostcenterdesc"])["_merch_abs_dev"].sum().reset_index()
            ...
            if merch_dev is not None:
                df_trend = df_trend.merge(merch_dev, on=["calendardate", "parentcostcenterdesc"], how="left")
            ...
            if "duplicate_units" in df_trend.columns:
                agg_dict["duplicate_units"] = ("duplicate_units", "sum")
            if "merch_deviation" in df_trend.columns:
            ...
            with col_left3:
                if "merch_deviation" in daily.columns:
            ...
            with col_right3:
                if "duplicate_units" in daily.columns:
            ...
            with col_left4:
                if "website_units" in daily.columns:
